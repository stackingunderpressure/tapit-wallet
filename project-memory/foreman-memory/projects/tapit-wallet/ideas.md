...
Refinement 2026-09-03 (arena home = prototype-in-wallet-then-export DECIDED; privacy STILL OPEN -- the commitment itself leaks TIMING):
Operator decision (chip): arena screen lives as a thin prototype INSIDE tapit-wallet first (plugs into live signing + transport, proves the real relay round-trip fastest), then the portable engine exports to a standalone Beat-the-HODL site that signs through Tapit (Layer-2). Engine already portable (pure moveChain + truthScore + moveChannel with a transport seam), so choosing the forever-home later costs nothing.

Operator's sharp unresolved catch on privacy (do NOT consider commit-reveal settled): "We got away from where the user is going to the public relay and using the QR to sign and keeping their own proof, and proving it at the end or not. If it's a certain kind of Nostr event and they can see that someone SIGNED something, that's a commitment -- will they know? Even though they can't see WHAT it is, they might INFER it stands out that they're probably moving their ball, and so they'll move theirs." The insight: hiding the CONTENT of a move (commit-reveal) does NOT hide the EVENT of moving. A visible commitment on a public relay still leaks TIMING -- a watcher who sees you post a sealed commitment right now can infer you're acting and front-run/follow on the mere fact, without ever reading the move. So commit-reveal solves content-copying but NOT timing-copying. This partially defeats the copy-trading protection and reopens the question.

Design directions this opens (for the resurface, NOT yet chosen):
- KEEP-YOUR-OWN-PROOF, PROVE-AT-THE-END (the operator's other framing): don't broadcast anything live at all; the user signs each move with their own key (QR/self-held) and KEEPS the signed chain privately; the ONLY public act is OPTIONAL, at the end, when they choose to reveal the whole finished chain to prove their record. No live event = no timing leak = no copy-trading, at the cost of losing the live public "didn't backdate" guarantee. But the chain's internal hash-linkage + sequence already makes backdating within the chain hard; the weaker claim is only "when did the FIRST commitment enter the world," which a single end-anchor (one OpenTimestamps/Bitcoin anchor of the whole chain at reveal) can cover.
- DECOY / BATCH TIMING COVER: everyone's commitments blend (fixed broadcast cadence, or many players committing in the same window) so an individual's timing doesn't stand out -- harder, needs volume, probably not v1.
- ANCHOR-NOT-BROADCAST: instead of a live Nostr broadcast per move, anchor each move (or the chain) to Bitcoin via OpenTimestamps (tapit-attest already has this) -- a Bitcoin anchor proves time WITHOUT a per-move public relay event a watcher is subscribed to; the proof exists on chain to be checked later, not pushed to followers in the moment. This may be the cleaner "public timestamp without a live copy-trading signal" than a Nostr event.
- MAKE-PUBLIC-GOOD reframing still available for the REVEALED record: a finished, revealed chain being fully public IS the reputation value; the fight is only about the LIVE phase.
Net: home is decided; the live-privacy model is the open design question, and the operator's leaning sounds like "keep your own proof, reveal at the end, don't push a live per-move event at all" -- lighter than commit-reveal and it dodges the timing leak. Resurface this as the first design fork before the arena screen's broadcast wiring is cut (the screen itself, the engine, and the scoreboard don't depend on which privacy model wins -- only the transport step does).

Refinement 2026-09-03 (arena proof-of-moment mechanism NAILED -> wrote ARENA_SPEC.md; four decisions LOCKED):
The whole "polish the proof of entry and exit" problem (flagged as the hardest part at the very start of this thread) is now specified end-to-end in ARENA_SPEC.md at the repo root. Four locked operator decisions (chip-answered):
1. GENESIS = a real on-chain donation to an OPEN-SOURCE CHARITY (not the arena/project). Public, timestamped, paid, non-escrow -- the start of the trail that legitimizes the claim; sats may be spent immediately, nothing downstream depends on them existing.
2. PRICE SOURCE = a SIGNED PRICE ORACLE (Bitcoin DLC-oracle pattern): oracle publishes {price,time,source,round} signed by its own key on a cadence; verification is pure math against the oracle pubkey, no "trust the chart"; the oracle cadence is the game's tick.
3. ANTI-HINDSIGHT FENCE = a LIVE PER-MOVE BITCOIN ANCHOR (OpenTimestamps): each move's hash anchored to Bitcoin at the moment = the latest-time bound that stops hindsight backfill; paired with the oracle's signed time (earliest bound). Operator accepted the mild timing-observability of the anchor over the more complex decoy-heartbeat and over an earliest-only honest-limit v1.
4. HOME = prototype the arena screen INSIDE the wallet first (fastest live-relay proof), then export the portable engine to a standalone site that signs through Tapit (Layer-2).
Ripples captured in the spec: moveChannel (kind 9584) changes role from live-move-broadcast to REVEAL channel (the live public act is the OTS anchor, not a Nostr move); move-chain gains an oracle-datum field + verifyMoveChain gains oracle-sig + oracle-time<=anchor-time checks (additive); anchoring is already proven (journal feature anchors via tapit-attest OTS primitives + the anchor worker); truthScore unchanged. OPEN pieces: the oracle (run our own transparent one vs adopt an existing DLC/Nostr price oracle -- biggest unbuilt piece), the charity address pick, and the arena screen itself (the NEXT CUT -- screen+engine+scoreboard don't depend on the oracle/anchor wiring, only move-build+verify do, so the playable surface can be built now and the oracle/anchor wired as they land). Stage: fruiting body (a real build-contract spec shipped). Next design fork when it earns time: the oracle decision.

Refinement 2026-09-03 (arena SCREEN shipped in-wallet prototype; ORACLE research back -- no reliable public Nostr oracle exists):
ARENA SCREEN BUILT (src/features/arena/): the first playable surface. arenaChain.ts (pure, 8 tests) is the arena glue over the generic move-chain -- buildGenesisDraft/buildSwitchDraft (every move tagged game='beat-the-hodl'), findArenaChain (pull the run out of holdings by tag+owner, ordered), nextSeq/headLink/nextSide (legal-move rule reuses the tested scorer's holding state so it can't drift). ArenaScreen.tsx: start a run (genesis, charity txid + stake optional in the prototype), sell-all/buy-all-back at a hand-entered price, live scoreboard from simulateWholeCoin (coins vs 1.0, edge, friction, min-buy-back-to-beat, holding, chain verified/broken), an optional "Publish to Nostr" reveal via publishPublicNote (proves the relay round-trip), and Clear-run. Each move is wallet.attest'd, held, queued for a Bitcoin anchor via anchorQueue (the SAME live per-move OTS anchor path the journal already uses -- so decision #3 from ARENA_SPEC is actually wired, not stubbed). Mounted as a standalone /arena route (HomeScreen is at the 800-line cap, and a route is the honest pre-cursor to the eventual export to a standalone site) with a launcher in Settings > More ("Beat the HODL (prototype)"). All six gates green: typecheck, lint, 937 tests (98 files, +8 arena), build (ArenaScreen chunk 4.26KB gz, under budget), baseline re-locked 98/934, doctrine. NOT browser-tested from the sandbox -- a device smoke (start/sell/buy, scoreboard in coins, persistence across reload, the Nostr publish) is owed.
PROTOTYPE SEAMS still open (both parallel, neither blocked the screen): (1) PRICE is hand-entered + stamped price_source='manual'; (2) GENESIS is a local start move, not yet the real on-chain charity donation.

ORACLE RESEARCH (background agent, grounded ~Sept 2026) -- IMPORTANT, changes the plan: there is NO reliable, public, Nostr-native BTC/USD price oracle to just consume today. The standard EXISTS (NIP-88, kind 88/89, DLC announcement+attestation, base64 TLV, BIP340 Schnorr -- browser-verifiable with @noble) and the reference oracle software exists (kormir, with an optional nostr feature), BUT the NIP is unmerged/stalled (PR #919 draft since Jan 2025; reduced-scope #1681 CLOSED June 2026 by the author; successor #2394 early) with NO live oracle npub named anywhere. Suredbits (the canonical DLC oracle) looks DEAD -- oracle.suredbits.com did not resolve. Vitor Pamplona's simpler "asset prices" signed-event NIP (#1658) is also just an open PR, no publisher. So "use existing, Nostr-native, reliable" cannot ALL be satisfied at once right now. Options: (A) RECOMMENDED -- a tiny self-signed round: a ~30-line signer (our key) emits a Nostr-EVENT-SHAPED signed {price,time,source,round,pubkey,sig} off an exchange candle, verified in-browser with @noble/curves schnorr + @noble/hashes sha256; it's the DLC-oracle property (anyone verifies against a known pubkey) WITHOUT depending on a dead/unmerged public oracle, and being Nostr-shaped it swaps to a real NIP-88 feed later with ZERO client change. (B) Pyth Hermes (hermes.pyth.network, BTC/USD) -- reliable + maintained, but its signature is a Wormhole guardian VAA meant for on-chain verify, not a clean one-pubkey browser Schnorr check; consume as a trusted number + stored receipt, not a browser-verified sig. (C) plain exchange candle over TLS (mempool.space/Binance/Coinbase/Kraken) -- unsigned, pragmatic, what WealthStrategy already does; the unsigned-ness is exactly the gap an oracle fills. (D) wait for a real NIP-88 publisher (kind 89 subscribe + TLV parse + @noble verify) -- only once a specific live maintained npub is confirmed, which none is today. Honest reconciliation of the operator's "existing, Nostr, reliable": pick ONE priority -- if "verifiable against a pubkey" matters most, (A) is the smallest honest build and stays Nostr-shaped; if "zero self-hosting" matters most, (B)/(C). This is the next arena design fork; surface as chips. Sources in the agent report (NIP-88 PR #919, #1681, #1658; kormir; Pyth Hermes docs; bitcoinops DLC topic).

Refinement 2026-09-03 (oracle DECIDED + BUILT -- "our own tiny signed round"):
Operator chose (chip) the own-signed-round path over Pyth / plain exchange candle. BUILT this session:
- netlify/functions/price-oracle.mts -- v2 Netlify function at /api/price-oracle; fetches BTC/USD (Coinbase, Kraken fallback) and signs a canonical round with the oracle's BIP340 key from ARENA_ORACLE_PRIVATE_KEY (Netlify-env secret, never committed, never VITE_-prefixed). Signs with @noble/curves schnorr directly (new root dep, was already transitive) over the SHARED roundDigest.
- src/features/arena/priceRoundCanonical.ts -- the ONE canonical serialization ([0,'price-round',round,time,source,price] -> sha256), @noble/hashes only so both the Vite bundle and the function bundle import it; a test pins the exact bytes so signer/verifier can't drift.
- src/features/arena/priceRound.ts -- SignedPriceRound type, signPriceRound (reference), verifyPriceRound (tapit-attest verifySignature -- the one-library Schnorr, no new verify crypto), fetchSignedRound (fetch + verify-or-throw). 6 tests (canonical bytes pinned, round-trips, wrong-oracle/tamper/junk all rejected).
- Wired into ArenaScreen behind env: arenaOracle() in src/shared/lib/env.ts reads VITE_ARENA_ORACLE_URL + VITE_ARENA_ORACLE_PUBKEY (optional, non-throwing). When set, a "Fetch signed price" button fetches+verifies+fills the price and a verified round stamps oracle_pubkey/sig/round/time/source into the move (via buildSwitchDraft's new `round` field) so the price is later re-verifiable; a hand-edited price clears the round and falls back to a manual stamp. Absent -> the manual price path is unchanged. netlify.toml gained a [functions] block; .env.example documents all three vars (server secret name only).
Output is Nostr-event-shaped ON PURPOSE: if a reliable NIP-88 / signed-price-event oracle ever appears, it drops into the same verify path with no client change (the research found none reliable in 2026, which is why we self-sign). Why self-signing is still honest per the mission: we invented no crypto (BIP340 + sha256, tapit-attest's own primitives); the oracle is a small transparent service whose pubkey is public and whose every round is independently checkable -- swappable and verifiable, not a trusted black box. All six gates green (943 tests, 99 files; ArenaScreen chunk 4.82KB gz; baseline re-locked 99/940). OWED (operator, can't smoke from sandbox): generate the oracle key (tapit-attest generateKeypair or any BIP340 keygen), set ARENA_ORACLE_PRIVATE_KEY in Netlify, publish the matching pubkey as VITE_ARENA_ORACLE_PUBKEY + set VITE_ARENA_ORACLE_URL, deploy, hit /api/price-oracle and confirm the round verifies in the arena. The Netlify function is NOT covered by tsc/eslint (they scan src only) -- it's esbuild-bundled at deploy; the shared canonical module + the verifier ARE fully tested. Stage: fruiting body.

Refinement 2026-09-04 (operator smoked the arena on device -- "looking pretty good"; hardened per feedback):
Operator: "set the friction at 2% and explain why ... pull in the oracle whatever we gotta do ... hand enter in the price by yourself is not gonna work ... make that seamless and super accurate and not capable to make it worth anything." Done:
- FRICTION fixed at 2%/leg (DEFAULT_FRICTION_PCT 0.5 -> 2), NOT user-lowerable, with an on-screen explainer (each leg pays fee + spread + slippage; ~4% round trip; pessimistic on purpose so a real win is real; can't be dialed down or you'd manufacture a fake win). The friction INPUT was removed from the UI entirely.
- ORACLE IS NOW THE PRICE SOURCE, not hand entry. ArenaScreen rewritten oracle-first: when an oracle is configured there is NO price field -- the Sell/Buy button EXECUTES at a freshly fetched, signature-verified, non-stale signed price at the instant of the tap ("...at the live price"), so the player never picks a flattering price. Live price is shown (value + source + age), auto-fetched when a run exists, with a Refresh. Hand entry survives ONLY as an explicit PRACTICE mode (amber "not verified" banner) when no oracle is set.
- ANTI-MANIPULATION HARDENING: (1) staleness -- priceRound.isRoundFresh + fetchSignedRound reject any round older than 120s (or future), so an old favourable signed price can't be replayed. (2) accuracy -- the oracle function now takes the MEDIAN of Coinbase + Kraken + Bitstamp, drops failed feeds, requires >=2, and rejects if min/max disagree >2%, so no single feed (glitch/outage/manipulated book) moves the signed number; the signed `source` names which feeds counted.
- GO-LIVE MADE EASY: scripts/arena-oracle-keygen.mjs generates the BIP340 pair and prints the exact env to set (ARENA_ORACLE_PRIVATE_KEY as a Netlify secret, VITE_ARENA_ORACLE_PUBKEY + VITE_ARENA_ORACLE_URL public) + verify/rotate steps.
All six gates green (944 tests, 99 files; ArenaScreen 5.33KB gz; baseline 99/941). STILL OWED (operator, can't do from sandbox): run the keygen, set the Netlify secret + the two VITE_ vars, deploy, hit /api/price-oracle to confirm the signed round verifies, then the arena flips from practice to live automatically. NEXT genuine wire after that: the real charity-donation genesis (replace the local start move with a real on-chain donation txid rooting the trail). Stage: fruiting body.

Refinement 2026-09-04 (arena redesigned to COPY WealthStrategy's Stones game -- real candle chart; friction 2% total; oracle no longer hand-entered):
Operator got into the arena ("looking pretty good") then directed: friction 2% total (= 1%/leg, done), "pull in the oracle whatever we gotta do -- hand enter the price yourself is not gonna work," "good visual and good solid execution," and then precisely: "the same game that inspired this game -- it's in the wealth bitcoin tab -- copy it." That game is WealthStrategy's Stones lab (src/components/stones/StoneChart.tsx + StonesTab.tsx, TradingView lightweight-charts). COPIED it into the arena:
- lightweight-charts@4.2.3 added as a dep. src/features/arena/ArenaChart.tsx = WealthStrategy's StoneChart copied verbatim, adapted (inline dark/orange palette instead of their theme import, local Candle type). Lazy-loaded so the ~53KB gz chart lib is its own deferred chunk (arena screen chunk stays 5.8KB); named budget added.
- src/features/arena/useBtcCandles.ts + netlify/functions/btc-candles.mts = WealthStrategy's candle feed copied. THE KEY WIN for "it's not working": this feed is multi-provider Kraken -> Coinbase -> Bitstamp public OHLC, NO API KEY, so it works on deploy where the oracle secret path didn't. Adapted the function to drop @netlify/blobs (no dep) -- fresh fetch per cold request + client 5-min cache + HTTP cache header.
- ArenaScreen REWRITTEN Stones-style: a dark Bitcoin-orange card with the live BTC/USD price (monospace orange, WealthStrategy look) + a 1h/4h/1d/1w interval toggle + the real candle chart with your sell/buy pins drawn on it (markers built from the signed move chain's price_time+kind+price); the sats-vs-1.0 scoreboard below; one big Sell/Buy button that executes AT THE MARKET PRICE (candle lastClose), never hand-typed -- so "hand enter is not gonna work" is satisfied by the market feed itself. Friction fixed 2% round trip, applied silently (no knob).
- THE ORACLE is now the OPTIONAL signing/proof layer, not the price source: if VITE_ARENA_ORACLE_URL/PUBKEY are set, a move executes at the freshly signed+verified oracle round (on-chain proof, priceSource='oracle'); if not, it executes at the candle lastClose (priceSource='market', unverified but fully playable). This decouples "playable + good-looking" (candles, no key) from "cryptographically verifiable price" (oracle, needs the key+deploy) -- the arena WORKS immediately on deploy now regardless of the oracle.
All six gates green (typecheck, lint, 944 tests, build with budgets OK incl. the new deferred ArenaChart chunk, baseline 99/941, doctrine). NOT browser-tested from the sandbox -- device smoke owed: the candle chart renders, the interval toggle works, the price reads live, sell/buy pins land on the chart, the scoreboard reads in coins. The move-chain/truthScore/priceRound engines are unchanged and still fully tested. Stage: fruiting body.
