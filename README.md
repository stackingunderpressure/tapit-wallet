...
## 📜 Open source, and the trust framework

MIT licensed — see [`LICENSE`](LICENSE). The two libraries under the hood carry
their own copies of the same license and are meant to be **lifted out and
reused**: [`tapit-attest/`](tapit-attest) is the standalone signed-attestation
primitive (BIP340 Schnorr, Merkle field tree, OpenTimestamps anchoring, Shamir
recovery, succession chains, the `Wallet` core), and
[`bip341-psbt-signer/`](bip341-psbt-signer) is a focused Taproot / PSBT signer.
Fork them, use them, build your own bridge — that's what they're for. We
invented none of the cryptography; the only new thing is composing it so an
ordinary person can use it without understanding it first.

[**`TRUST_FRAMEWORK.md`**](TRUST_FRAMEWORK.md) is the forkable playbook for the
other half of the thesis: how a town, a church, or any small group forms its
own signature and starts vouching for its own people — no group key, no
permission, just accreted signatures anyone can verify. Mechanism plus a
playbook; the group runs its own thing.
