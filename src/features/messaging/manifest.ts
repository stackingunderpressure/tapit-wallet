...
export const manifest: FeatureManifest = {
  ...
  monetizable: false,
  notes:
    'WalletProvider owns the per-peer thread map and the sendChatMessage callback; messaging consumes both via useWallet. The chat subscription runs alongside the existing inbox subscription inside WalletProvider transport useEffect — same transport handle, separate handler, independent kind filter (NIP17_GIFT_WRAP_KIND = 1059 vs TAPIT_ENVELOPE_KIND = 9573). Outbound chat optimistically appends to the local thread before publish settles so the composer feels instant; if publish fails the message stays in the thread but a future polish cut will mark it as undelivered. ConnectionCard gains an optional onOpen prop; ClassicConnections + FreshCrew pass it through. removal_safe: false because HomeScreen imports PeerThread. NIP-17 chat migration (2026-05-26 evening, see transport manifest for the rationale): the wire format moved from custom kind 9574 to NIP-17 gift-wrapped kind 1059 because public relays were accept-but-not-persist on the custom kind, leaving recipients offline-then-online unable to retrieve messages. The migration kept the messaging feature\'s consumer surface (sendChatMessage callback + chatThreadsByPeer Map) unchanged — only the encryptedInbox.ts helpers underneath changed wire shape. Threads continue to track by the REAL peer pubkey (recovered from the seal\'s signed pubkey, not the gift wrap\'s ephemeral wrapper) so the per-peer Map and the existing IDB persistence layer work without migration. Privacy upgrade: the relay no longer sees the real sender pubkey on each chat publish.',
};
